...
sedml = libsedml.readSedMLFromFile("../old_SEDML/Intosalmi2015.sedml")
...
yaxis=plot.createYAxis()
yaxis.setType(libsedml.SEDML_AXISTYPE_LINEAR)
yaxis.setName("Ror $\gamma$t mRNA")
yaxis.setGrid(False)
yaxis.setMin(0)
yaxis.setMax(0.6)
...
curve = plot.getCurve(0)
curve.setStyle("solid_blue")
curve.setName("Ror $\gamma$t mRNA")



plot = sedml.getOutput(2)

plot.setName("Figure 3c")
...
mod1.setSource("Intosalmi2015.xml")


sed = libsedml.writeSedMLToString(sedml)
te.saveToFile("Intosalmi2015.sedml", sed)
...
sedml_filenames = ["Intosalmi2015.sedml"]
archive = build_combine_archive(".", sedml_filenames)
combine_writer.run(archive, ".", "BIOMD0000001004.omex")
